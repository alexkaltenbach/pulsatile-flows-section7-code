...
with open('./Sym1/errors.pkl', 'rb') as datei:
...
for n in range( 1, 10 ):
    ...
    vL   = Function(V, "./Sym1/v" + str( n ) + ".xml")
    X0   = mesh.coordinates()
...
plt.style.use('bmh')
ax2.plot(X[:250],R[:250],c='tab:red',lw=1.5,ls='-')
ax2.plot(X[750:],R[750:],c='tab:red',lw=1.5,ls='-')
ax2.plot(X[250:750],R[250:750],c='tab:red',lw=1.5,ls='-')
