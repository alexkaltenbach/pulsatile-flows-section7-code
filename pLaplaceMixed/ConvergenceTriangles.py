...
def _triangle_extent(positive_x, rate, max_span, max_height_factor):
    x_left = positive_x[0]
    last_index = min(max_span, len(positive_x) - 1)
...
def draw_convergence_triangles(
    ...
    for curve in curves:
        ...
        rate = _rounded_rate(estimated_rate, step=rate_step)
        curve_data = {
            'color': curve['color'],
            'x': x_curve,
            'y': y_curve,
    ...
    for rate, group in sorted_groups:
        ...
        x_left, x_right, q = _triangle_extent(positive_x, rate, span, max_height_factor)
        y_low = max(_curve_lower_edge_requirement(curve, x_left, x_right, rate, vertical_gap)
                    for curve in group)
        y_low = max(y_low, y_ref * vertical_gap)
        y_base = y_low * (q ** rate)
        y_low = y_base / (q ** rate)
        candidate = {
        ...
        _add_boxed_label(
            ax,
            _rate_label(rate),
            (x_left, np.sqrt(y_low * y_base)),
            label_fontsize,
        )
        _add_boxed_label(
            ax,
            '1',
            (np.sqrt(x_left * x_right), y_base),
            label_fontsize,
        )
