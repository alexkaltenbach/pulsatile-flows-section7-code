...
with open('./NonSym2/errors.pkl', 'rb') as datei:
...
for n in range( 1, 10 ):
    ...
    vL   = Function(V, "./NonSym2/v" + str( n ) + ".xml")
    X0   = mesh.coordinates()
...
ax2.plot(X[750:],R[750:],c='tab:red',lw=1.5,ls='-')
#ax2.plot(X[(X>-0.5)&(X<=0.5)],R[(X>-0.5)&(X<0.5)],c='tab:red',lw=1.5,ls='-')
